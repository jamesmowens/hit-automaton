...
public class NegativeCachingv2_NestMegaSEQoperator extends
	...
	Hashtable<Integer, ArrayList<NegativeCache>> cacheResultsNeg = new Hashtable<Integer, ArrayList<NegativeCache>>();
	...
	public NegativeCachingv2_NestMegaSEQoperator(int operatorID,
...
//							.getCache().size(); i++) {
					for (int i = this.cacheResults.get(qi)
							.getCache().size()-1; i >= processedNum
					; i--) {
...
//													
										...
										if (cachefound == false) {
											...
											if(indexRemember>0)
												{
												if(childBuffer.isEmpty()&& cacheResultsNeg
														.get(childID)
														.get(indexRemember)
														.getRighttime() < timestampr)
													cacheResultsNeg.get(indexRemember).get(childID).setRighttime(timestampr);
												if(!childBuffer.isEmpty()&& cacheResultsNeg
														.get(childID)
														.get(indexRemember)
														.getRighttime() > timestampr)
											...
											{
												if(childBuffer.isEmpty())
													nc = new NegativeCache(leftTimebound,rightTimebound, true);
												else
													nc = new NegativeCache(leftTimebound,rightTimebound, false);
												if(cacheResultsNeg.get(childID)!=null)
												{
													cacheResultsNeg.get(childID).add(nc);
												}
												else
												{
													ArrayList<NegativeCache> temp = new ArrayList<NegativeCache>();
													temp.add(nc);
													cacheResultsNeg.put(childID,temp);
												}
