...
public class SemanticCaching_NestMegaSEQoperator extends SingleInputStreamOperator {
        ...
        Hashtable<Integer, ArrayList<CacheResults>> cacheList = new Hashtable<Integer, ArrayList<CacheResults>>();
        ArrayList<CacheInterval> cacheCheck = new  ArrayList<CacheInterval> ();
        ...
        public SemanticCaching_NestMegaSEQoperator(int operatorID, StreamQueue[] input,
                       StreamQueue[] output) {
                       ...
                       if (stackIndex >= 0 && AIS[stackIndex] != null) {
                               ...
                               for (int qIndex = 0; qIndex < rootIndex.size(); qIndex++) {
                                      ...
                                      if (eventType.equalsIgnoreCase(type)) {
                                              processQuery(rootIndex.get(qIndex), tuple, schArray,expiringTimestamp);
                                              System.gc();
//                                              System.out.println(cacheResults.get(2).getCache().size());
                                      }
                               }
                       ...
                       if (Configure.previousresultNum != Configure.resultNum) {
                               String write = Configure.resultNum + " "
                                              ...
                                              cacheResults.get(2).getCache().size() + " " + exactMatch + " " + partialMatch;

                               System.out.println(write);
//                               System.out.println(write + " " + cacheResults.get(2).getCache().size() + " " + exactMatch + " " + partialMatch) ;
//                               System.out.println()
...
//                       int nestPos, SchemaElement[] inputschArray, byte[] tempevent, double expiringTimestamp) {
//
...
//                                              timestampl, timestampr, expiringTimestamp);
...
//                                                     .getCache().size(); i++) {
                            	   for (int i = this.cacheResults.get(qi)
                                           .getCache().size()-1 ; i >= processedNum; i--) {
...
//                                              }
                                              ...
                                              */
                                              for (int childIndex = 0; childIndex < children.size(); childIndex++) {
                                                     ...
                                                     CacheInterval queryInterval = new CacheInterval();
                                                     queryInterval.setInterval(timestampl, timestampr);                              
//                                                     if(!cacheCheck.isEmpty())
//                                                     System.out.print(cacheCheck.get(queryInterval));
                                                     boolean match = false;
//                                                     for(CacheInterval c: cacheCheck)
//                                                     {
//                                                    	 if(c.getLeft()==timestampl && c.getRight()==timestampr)
//                                                    		 match = true;
////                                                    		 if(cacheCheck.containsKey(queryInterval))
//           
//                        
//                                                     
//                                                     }
...
//                                                     else{
//                                                    	 partialMatch++;
//                                                    	 cacheCheck.add(queryInterval);
//                                                     }
//                                                     childBuffer.clear();
                                                     if(cacheList.get(childID)!= null){
                                                    	 int index = 0;
                                                     for(CacheResults c: cacheList.get(childID))
                                                     {
                                                    	 
//                                                    	 double timestamp = StreamAccessor.getDoubleCol(tuple, schArray, 1);
//                                                         double expiringTimestamp = timestamp - Configure.windowsize;
//                                                         if(c.getLefttime()<expiringTimestamp)
//                                                        	 cacheList.get(childID).remove(index);
                                                    	 if(c.getLefttime()==timestampl && c.getRighttime()==timestampr)
                                                    		 match = true;
//                                                    		 if(cacheCheck.containsKey(queryInterval))
                                                    	 childBuffer = c.getCache();
                                                    	 index++;
                                                     ...
                                                     if(match)
                                                    	 ...
                                                    	 childBuffer = computeChildrenQueries(childID,
                                                                 ...
                                                                 inputschArray);
                                                    	 CacheResults temp = new CacheResults(childBuffer);
                                                    	 
                                                    	 temp.lefttime = timestampl;
                                                       	 temp.righttime = timestampr;
                                                       	 if(cacheList.get(childID)==null){
                                                       		ArrayList<CacheResults> cList = new  ArrayList<CacheResults> ();
                                                       		 cacheList.put(childID, cList);
                                                       		 
                                                       	 }
                                                       	 else
                                                       		 cacheList.get(childID).add(temp); 
                                                     }
                                                     }
                                                     else{
                                                    	 partialMatch++;
...
//                                                             childBuffer = computeChildrenQueries(childID,
//                                                                            cstackTypes, result, nestPos,
//                                                                            inputschArray, tempevent, expiringTimestamp);
//                                                             increment_flag = 1;
////                                                           exactMatch++;
                                                     } else {
...
//                                                                                    
        ...
        * 
         */
        void processQuery(int checkQIndex, byte[] tuple, SchemaElement[] schArray, double expiringTimestamp) {
