...
public class Flatten_MegaSEQoperator extends SingleInputStreamOperator {
	...
	public Flatten_MegaSEQoperator(int operatorID, StreamQueue[] input,
			StreamQueue[] output) {
	...
	public void classVariableSetup(String key, String value)
		...
		if (key.equalsIgnoreCase("query")) {
			...
			queries.add(new QueryInfo(Flatten_MegaSEQoperator.query_ID++,
					ETypes_AQuery));
	...
	public int run(int maxDequeueSize) {
		...
		for (int i = maxDequeueSize; i > 0; i--) {
			...
			/*
			 * if (timestamp == 25532.826) { System.out.print("check"); }
			 */
			...
			if (stackIndex >= 0 && AIS[stackIndex] != null) {
				...
				for (int iter = 0; iter < this.queries.size(); iter++) {
					{
						processQuery(iter, eventType, stackIndex, tuple,
								schArray);
					}
