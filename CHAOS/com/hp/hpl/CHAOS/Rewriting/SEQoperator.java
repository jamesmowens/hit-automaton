...
public class SEQoperator extends SingleInputStreamOperator {
	...
	public SEQoperator(int operatorID, StreamQueue[] input, StreamQueue[] output) {
	...
	ArrayList<ArrayList<byte[]>> connect(byte[] currenttuple,
			...
			EventActiveInstanceQueue[] stacks) {
	...
	/*ArrayList<PreviousTuples> previousTuple_HStacks_negated(int queryID,
		...
		if (table2 != null) {
			...
			*//**
			 ...
			 * ArrayList<String> stackTypesToCheck = new ArrayList<String>();
			 * stackTypesToCheck.add(e) ArrayList<Integer> checkingQid = new
			 * ArrayList<Integer>();
			 
			// for a particular previous tuple, some queries may already fail,
			// I need to record only the existing ones and early stop the others
			
			 * for (int j = 0; j < c_queries.size(); j++) { String types =
			 * c_queries.get(j).stackTypes.toString(); int c_start =
			 * types.indexOf(type); // current type int c_end =
			 * types.indexOf(positivePreviousTypes);//previous type
			 * types.subSequence(c_start, c_end);
			 * 
			 * }
			...
			while (tuple != null) {
				...
				for (int i = 0; i < previousTuples.size(); i++) {
					...
					earray3 = connect(tuple, sequenceConstruction_Hstacks(
							...
							inputschArray, tuple), inputschArray, stacks);
						...
						if (contains_notsensitive(
							...
							earray3 = connect(currenttuple,
									sequenceConstruction_Hstacks_negated(
											...
											currenttuple, stop), inputschArray,
									stacks);
							for (int t = 0; t < earray3.size(); t++) {
								earray.add(earray3.get(t));
							}
	...
	/*
	 * protected boolean checkPoints(byte[] leftTuple, byte[] rightTuple, int
	 * queryID, EventActiveInstanceQueue[] stacks, int stackIndex,
	 * SchemaElement[] inputschArray) {
	 * 
	 * boolean addQuery = false; double timeleft =
	 * StreamAccessor.getDoubleCol(leftTuple, inputschArray, 1); // double
	 * timeright = StreamAccessor.getDoubleCol(rightTuple, // inputschArray, 1);
	 * 
	 * ArrayList<PreviousTuples> previousTuples = previousTuple_HStacks_negated(
	 * queryID, stacks, stackIndex, rightTuple, inputschArray);
	 * 
	 * 
	 * // RIMM tuples without AMAT after it before INTC
	 * 
	 * boolean loop = true; byte[] previous = null; boolean outerLoop = true;
	 * double timeupperprevious = 0;
	 * 
	 * byte[] upperpreviousTuples;
	 * 
	 * for (int j = 0; j < previousTuples.size() && outerLoop; j++) { previous =
	 * previousTuples.get(j).getPreviousTuple();
	 * 
	 * double time_previous = StreamAccessor.getDoubleCol( previous,
	 * inputschArray, 1);
	 * 
	 * double timesp = previousTuples.get(j).getStopTimestamp();
	 * 
	 * //for all the tuples between time_previous and timesp
	 * 
	 * while (loop && previousTuples.size() != 0) {
	 * 
	 * String previousEventType = getTupleType(previous, inputschArray); //
	 * check if the event type is actually negative // (rewritten) if
	 * (contains_notsensitive(this.queries.get(0).stackTypes,
	 * previousEventType)) { // it is actually positive, we should connect it
	 * with // other events
	 * 
	 * loop = false;
	 * 
	 ...
	 * else {
	 * 
	 * // get the ORCL tuple before such RIMM // int stackIndextemp =
	 * findStack(previousEventType);
	 * 
	 * // get the RIMM tuple index, int index =
	 * StreamAccessor.getIndex(previous); byte[][] retPointerArrayTemp = new
	 * byte[2][]; // hard code // get the RIMM tuple above it byte[]
	 * currenttuple = stacks[stackIndextemp] .getPreviousByPhysicalIndex(index,
	 * retPointerArrayTemp);
	 * 
	 * // get the previous tuple before RIMM. upperpreviousTuples =
	 * previousTuple_HStacks_negated( queryID, stacks, stackIndextemp,
	 * currenttuple, inputschArray).get(0).getPreviousTuple();
	 * 
	 * timeupperprevious = StreamAccessor.getDoubleCol( upperpreviousTuples,
	 * inputschArray, 1);
	 * 
	 * previousTuples = previousTuple_HStacks_negated(queryID, stacks,
	 * stackIndextemp, previous, inputschArray); if (previousTuples != null &&
	 * previousTuples.size() > 0) { previous =
	 ...
	 * }
	 * 
	 * // till this point, previousTuples contain all the positive events if
	 * (previousTuples.size() != 0) {
	 * 
	 * for (int i = 0; i < previousTuples.size(); i++) { previous =
	 * previousTuples.get(i).getPreviousTuple();
	 * 
	 * double timeprevious = StreamAccessor.getDoubleCol(previous,
	 * inputschArray, 1);
	 * 
	 * if (timeleft <= timeprevious && timeleft >= timeupperprevious) addQuery =
	 * true; outerLoop = false; break; }
	 * 
	 * } }
	 * 
	 * return addQuery; }
	...
	public void produceinorder_HStacks(int queryID,
		...
		//	String previousEventType = getTupleType(previousTuple,
			...
			if (timestampPrevious > stopTime) {
				...
				//	double timestamp = StreamAccessor.getDoubleCol(ar2.get(0),
					//		inputschArray, 1);
					...
					{
						bufferResult(this.queries.get(0).queryID, dest,
	...
	// get the root query size, all positives
	protected int getResultSize() {

		return this.queries.get(0).stackTypes.size();
	...
	public int run(int maxDequeueSize) {
		...
		for (int i = maxDequeueSize; i > 0; i--) {
			...
			/*
			 * if (timestamp == 34206.611) { System.out.print("check"); }
			...
			if (stackIndex >= 0 && AIS[stackIndex] != null) {
				...
				ArrayList<String> stackTypes = this.queries.get(0).stackTypes;
			...
			{
				String write = Configure.executionTime + " "
						+ Configure.resultNum;
			...
			//String write = Configure.inputConsumed + " " + Configure.executionTime + " " + Configure.resultNum; 
			//resultCollection.add(write);
	...
	void processQuery(int checkQIndex, byte[] tuple, SchemaElement[] schArray) {
		...
		produceinorder_HStacks(queryID, AIS, tuple, schArray);
