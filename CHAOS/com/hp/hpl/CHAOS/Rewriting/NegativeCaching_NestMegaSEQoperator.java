...
public class NegativeCaching_NestMegaSEQoperator extends
		SingleInputStreamOperator {
	...
	Hashtable<Integer, ArrayList<CacheResults>> cacheResultsNeg = new Hashtable<Integer, ArrayList<CacheResults>>();
	...
	public NegativeCaching_NestMegaSEQoperator(int operatorID,
	...
	public int init(SchemaElement[] schArray) {
		...
		for (int i = 0; i < types.size(); i++) {
			...
			stack = new EventActiveInstanceQueue(schArray, pointerSize,
					types.get(i));
		...
		if (isFirst) {
			...
			while (tuple != null) {
				...
				for (int i = 0; i < previousTuples.size(); i++) {
					...
					earray3 = connect(
							tuple,
							sequenceConstruction_Hstacks(sIndex,
	...
	ArrayList<ArrayList<byte[]>> sequenceConstructionAnd(int queryID,
		...
		if ((traverseStackIndex == triggerStack) && (traverseStackIndex != 0)) {
			...
			earray3 = connectAnd(
					stacks[traverseStackIndex].eventQueue.peekLast(),
					sequenceConstructionAnd(queryID, previousStackIndex,
							triggerStack, stacks, leftTimebound,
		...
		if (isFirst) {// ok. bug place, instead of "0", I should say whether it
			...
			while (currenttuple != null
				...
				if (currentTupleTimestamp <= rightTimebound) {
					earray3 = connectAnd(
							currenttuple,
							sequenceConstructionAnd(queryID,
									previousStackIndex, triggerStack, stacks,
		...
		for (int i = 0; i < previousTuples.size(); i++) {
			...
			for (int k = 0; k < sc.size(); k++) {
				...
				double timestamp = StreamAccessor.getDoubleCol(ar2.get(0),// why
						// first
						inputschArray, 1);
				// if (timestamp > leftTimebound) // mo-medhabi thought
				{
					SchemaElement[] schArray_Result = generateResultSchemas(
			...
			if (cache == null) {
				...
				if (ar2.size() != 0) {
					if (this.cacheResults.get(queryID).righttime < StreamAccessor
							.getDoubleCol(ar2.get(ar2.size() - 1), schArray, 1)) {
					...
					/*
					 * if (cacheResults.get(queryID).righttime == 34201.581) {
					 * System.out.println("bug place"); }
					 */
					// test end
					...
					if (!this.cacheResults.get(queryID).getCache()
							.contains(ar2)) {
						this.cacheResults.get(queryID).getCache().add(ar2);
					}
	...
	public int run(int maxDequeueSize) {
		...
		for (int i = maxDequeueSize; i > 0; i--) {
			...
			if (expiringTimestamp >= 0) {
				...
				// purge caching results

				if (this.cacheResults != null)

					Utility.purgeResultCache(this.cacheResults,
							expiringTimestamp, schArray);
				if (this.cacheResultsNeg != null)

					Utility.purgeResultCacheList(this.cacheResultsNeg,
							expiringTimestamp, schArray);
			...
			if (timestamp == 35148.521) {
				...
				/*
				 * Set<Integer> set12 = resultBuffers.keySet();
				 * 
	...
	protected ArrayList<ArrayList<byte[]>> computeChildrenQueries(int childID,
			...
			int nestPos, SchemaElement[] inputschArray, byte[] tempevent) {
...

		{
			timestampr = StreamAccessor.getDoubleCol(result.get(nestPos),
					inputschArray, 1);
			double lastEvent = StreamAccessor.getDoubleCol(
					result.get(result.size() - 1), inputschArray, 1);
	...
	protected ArrayList<ArrayList<byte[]>> compute(int qi,
		...
		Hashtable<Integer,  CacheResults> childrenBuffersNegative = new Hashtable<Integer,  CacheResults>();

		ArrayList<ArrayList<byte[]>> childBuffer = new ArrayList<ArrayList<byte[]>>();
		CacheResults childBufferNeg = new CacheResults(childBuffer, rightTimebound, rightTimebound);
		...
		if (children.size() != 0) {
			...
			{
				processedNum = 0;
				if (this.cacheResults.get(qi) != null && this.cacheResults.get(qi).getCache()!=null)
...
//							.getCache().size(); i++) {
					for (int i = processedNum; i < this.cacheResults.get(qi)
						...
						double timestampSecond = 0;
						double timestampLast = 0;

						if (result.size() == 3) {
							byte[] firstTuple = result.get(0);
							timestampFirst = StreamAccessor.getDoubleCol(
									firstTuple, inputschArray, 1);

							byte[] secondTuple = result.get(1);
							timestampSecond = StreamAccessor.getDoubleCol(
									secondTuple, inputschArray, 1);

							byte[] lastTuple = result.get(2);
							timestampLast = StreamAccessor.getDoubleCol(
									lastTuple, inputschArray, 1);

							/*
							 * if (timestampFirst == 34198.925 &&
							 * timestampSecond == 34201.581 && timestampLast ==
							 * 34201.589) {
							 * 
							 * System.out.println("bug place"); }
							 */
						...
						/*
						 * if (cacheResults.get(2) != null) if
						 * (cacheResults.get(2).righttime == 34201.581)
						...
						for (int childIndex = 0; childIndex < children.size(); childIndex++) {
							...
							int posType = children.get(childIndex)
							...
							if ((cacheResults.get(childID)) == null
								...
								if (posType == 0) {
									if (cacheResults.get(childID).righttime >= timestampr) {
										...
										double t = StreamAccessor.getDoubleCol(
												tu, inputschArray, 1);
										int tupleIndex;

										// small adjustment.
										// need debug binary search right bound
										if (t > timestampr) {// I give you
																// another
											// chance to see
										...
										while (tu != null && t >= localtime
											...
											tempchildBuffer = compute(childID,
													AIS, tu, inputschArray,
													timestampl, timestampr);
								...
								//negative caching
									...
									if (!cacheResultsNeg.isEmpty()) {
										if (!cacheResultsNeg.get(childID)
												.isEmpty()) {
											for (int index = 0; index < cacheResultsNeg
													.get(childID).size(); index++) {

												if (cacheResultsNeg
														.get(childID)
														.get(index)
														.getLefttime() == timestampl
														&& cacheResultsNeg
																.get(childID)
																.get(index)
																.getRighttime() == timestampr) {
													childBuffer.clear();
													childBuffer = cacheResultsNeg
															.get(childID).get(index).getCache()
															;
													childBufferNeg.setCache(childBuffer);
													childBufferNeg.setLefttime(timestampl);
													childBufferNeg.setRighttime(timestampr);
													cachefound = true;
													break;
											...
											if (cachefound == false) {
												childBuffer.clear();
												childBuffer = computeChildrenQueries(
														childID, cstackTypes,
														result, nestPos,
														inputschArray,
														tempevent);
												CacheResults cache = new CacheResults(
														childBuffer,
														timestampl, timestampr);
												cacheResultsNeg.get(childID)
														.add(cache);
												childBufferNeg.setCache(childBuffer);
												childBufferNeg.setLefttime(timestampl);
												childBufferNeg.setRighttime(timestampr);
										...
										else
										{
											childBuffer.clear();
											childBuffer = computeChildrenQueries(
													childID, cstackTypes, result,
													nestPos, inputschArray,
													tempevent);
											CacheResults cache = new CacheResults(
													childBuffer, timestampl,
													timestampr);
											ArrayList<CacheResults> cacheList = new ArrayList<CacheResults>();
											cacheList.add(cache);
											cacheResultsNeg.put(childID, cacheList);
											childBufferNeg.setCache(childBuffer);
											childBufferNeg.setLefttime(timestampl);
											childBufferNeg.setRighttime(timestampr);
										}
							...
							if (posType == 1 && childBuffer.size() != 0)
							{
//								childrenBuffers.put(arg0, arg1)
								childrenBuffersNegative.put(new Integer(childID),
										childBufferNeg);
							}
							if (posType == 0 && childBuffer.size() != 0) {
								...
								/*
								 * if (cacheResults.get(childID).righttime ==
								 * 34201.581) { System.out.println("bug place");
								 * }
								 */
								// test end
								// childBuffer.clear();
							...
							for (int childIndex = 0; childIndex < children
								...
								if (childIndex == 0) {
									if (type == 0) {
										...
										CacheResults bufferedTuple = new CacheResults(new ArrayList<ArrayList<byte[]>>(),0.0,0.0);
										if(!childrenBuffersNegative.isEmpty())
											bufferedTuple = childrenBuffersNegative
												.get(cID);
//										if(childrenBuffers.get(cID)!=null)
//										childrenBuffers.get(cID).clear();
//										if (bufferedTuple != null
//												&& bufferedTuple.size() != 0)
										if(!bufferedTuple.getCache().isEmpty())
										{
											stop = true;
											break;
									...
									for (int pI = 0; tempBuffers != null
										...
										} else if (type == 1)// negative, filter
										// results
										{
	...
	protected int getNestedStackNum(int qi) {
		...
		if (childrenq.size() != 0) {
			for (int i = 0; i < childrenq.size(); i++) {
				...
				if (type == 0)// positive event types
