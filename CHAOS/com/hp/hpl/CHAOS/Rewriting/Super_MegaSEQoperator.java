...
public class Super_MegaSEQoperator extends SingleInputStreamOperator {
	...
	QueryInfo[][] orderedunclusterQueries;
	...
	ArrayList<QueryInfo> clusteredQueries = new ArrayList<QueryInfo>();// using
	// shared
	ArrayList<QueryInfo> unclusteredQueries = new ArrayList<QueryInfo>();// using
	// topdown/naive
	...
	CostModel CM = new CostModel(orderedunclusterQueries, tree);
	...
	public Super_MegaSEQoperator(int operatorID, StreamQueue[] input,
			StreamQueue[] output) {
	...
	public int init(SchemaElement[] schArray) {
		...
		// set up clustered queries from the queries ArrayList
		for (int r = 0; r < this.queries.size(); r++) {
			int qID = this.queries.get(r).getQueryID();
			ArrayList<Integer> clusteredIDs = this.queries.get(r)
					.getClusteredQueryIDs();
			if (clusteredIDs.size() != 0) {
				clusteredQueries.add(this.queries.get(r));

				for (int clusterIDIndex = 0; clusterIDIndex < clusteredIDs
						.size(); clusterIDIndex++) {
					clusteredQueries.add(getQuery(clusteredIDs
							.get(clusterIDIndex)));
				}
		...
		// set up unclustered queries from the queries ArrayList
		for (int r = 0; r < this.queries.size(); r++) {

			QueryInfo q = getQuery(this.queries.get(r).getQueryID());
			if (!clusteredQueries.contains(q)) {
				unclusteredQueries.add(q);
			}
		}

		this.orderedunclusterQueries = orderQueries(this.unclusteredQueries);

		CM.setOrderedQueries(this.orderedunclusterQueries);
		setupExecutionOrder();
	...
	public void setupExecutionOrder() {
		for (int i = 0; i < this.orderedunclusterQueries.length; i++) {
			int currentqID = this.orderedunclusterQueries[i][0].queryID;

			double singleCost = this.CM.singleCompute(currentqID);
			double fromTopCost = Double.MAX_VALUE;

			double min = singleCost;
			this.orderedunclusterQueries[i][0].setComputeSourceID(currentqID);

			// we only have two choices in the top down evaluation: from itself
			// or from the top query
			if (this.orderedunclusterQueries[i][1] != null) {
				int parentqID = this.orderedunclusterQueries[i][1].queryID;
				...
				if (fromTopCost - 100 < min) {// Here, I relax the constraint,
					...
					// indeed better
					min = fromTopCost;
					this.orderedunclusterQueries[i][0]
							.setComputeSourceID(parentqID);
				}
			}
	...
	public int run(int maxDequeueSize) {
		...
		for (int i = maxDequeueSize; i > 0; i--) {
			...
			/*
			 * if (timestamp == 34178.648) { System.out.print("check"); }
			 */
			/*
			...
			if (stackIndex >= 0 && AIS[stackIndex] != null) {
				...
				{
					// iterate all root queries in clustered queries
					for (int qIter = 0; qIter < this.clusteredQueries.size(); qIter++) {
						if (this.clusteredQueries.get(qIter).isRoot()) {
							processQuerySharedNormalized(this.clusteredQueries
									.get(qIter).getQueryID(), tuple, schArray,
									expiringTimestamp);
						}
					}

					// iterate all un-clustered queries
					for (int checkQIndex = 0; checkQIndex < this.orderedunclusterQueries.length; checkQIndex++) {

						if (this.orderedunclusterQueries[checkQIndex][0].computeSourceID == this.orderedunclusterQueries[checkQIndex][0].queryID) {
							// regular pointer join for self computation
							processQuery(checkQIndex, eventType, stackIndex,
									tuple, schArray);
						} else// compute from the upper level query
						{
							processQueryTopdown(checkQIndex, schArray, tuple);
			...
			if (Configure.previousresultNum != Configure.resultNum) {
				...
				System.out.println(write);
                                ...
                                try {
                                        FileWriter fstream = new FileWriter("Rewriting.txt", false);
	...
	void processQuery(int checkQIndex, String eventType, int stackIndex,
		...
		ArrayList<String> stackTypes = orderedunclusterQueries[checkQIndex][0].stackTypes;
		String type = stackTypes.get(stackTypes.size() - 1);
		...
		if (semanticMatch(type, eventType)) {
			int queryID = this.orderedunclusterQueries[checkQIndex][0].queryID;
	 ...
	 * 
	 * @param checkQIndex
	 *            : note, it is the index in orderedunclusterQueries, it is not
	 *            the queryID
	 */
	protected void processQueryTopdown(int checkQIndex,
			SchemaElement[] schArray, byte[] tuple) {
		...
		if (eventType.equalsIgnoreCase(type)) {
			int upperqID = this.orderedunclusterQueries[checkQIndex][0].computeSourceID;
			int downqID = this.orderedunclusterQueries[checkQIndex][0].queryID;

			double visitedLastTime = -1;
			if (checkVisited(upperqID, downqID)) {
				visitedLastTime = this.existingResultNums.get(upperqID).get(
						downqID).doubleValue();
			...
			if (!this.resultBuffers.isEmpty()
				...
				topdown_enqueueSubsequences(upperqID,
						this.orderedunclusterQueries[checkQIndex][0].queryID,
	...
	public void topdown_bufferResult(int queryID, byte[] dest,
		...
		if (Utility.windowOpt(ar2, schArray)) {
			...
			boolean reuse = false;
			for (int k = 0; k < this.orderedunclusterQueries.length; k++) {
				if (this.orderedunclusterQueries[k][0].computeSourceID == queryID) {
					reuse = true;
					break;
				}
			}
	...
	public ArrayList<ArrayList<byte[]>> produceinorder_HStacks_topdown(
			ArrayList<String> stackTypes, EventActiveInstanceQueue[] stacks,
			int stackIndex, byte[] tempevent, SchemaElement[] inputschArray,
			int eventTypeNum) {
		...
		if (lastCqType.equalsIgnoreCase(lastqType)) {
			...
			if (Utility.getPositiveTypeNum(nonexistTypes) == 1) {
				...
				while (offTuple != null) {
					...
					nonTypesresults = produceinorder_HStacks_topdown(
	 ...
	 * 
	 */
	void processQuerySharedNormalized(int queryID, byte[] tuple,
			SchemaElement[] schArray, double timestampleft) {
		String eventType = getTupleType(tuple, schArray);
		// I assume the last event type is positive.
		ArrayList<String> QueryTypes = getQuery(queryID).stackTypes;

		if (eventType.equalsIgnoreCase(QueryTypes.get(QueryTypes.size() - 1))) {
	 ...
	 * @return
	 */
	protected QueryInfo[][] findParents(ArrayList<QueryInfo> queries) {
		// initialize array
	 ...
	 */
	public QueryInfo[][] orderQueries(ArrayList<QueryInfo> queries) {
		QueryInfo[][] queryArray = findParents(queries);
