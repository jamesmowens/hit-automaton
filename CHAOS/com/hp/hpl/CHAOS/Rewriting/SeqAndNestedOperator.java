 ...
 * 
 */
public class SeqAndNestedOperator extends SingleInputStreamOperator {
	...
	public SeqAndNestedOperator(int operatorID, StreamQueue[] input,
			StreamQueue[] output) {
	...
	public int run(int maxDequeueSize) {
		...
		for (int i = maxDequeueSize; i > 0; i--) {
			...
			/*
			 * if (timestamp == 34205.968) { System.out.print("check"); }
			 */
	...
	ArrayList<ArrayList<byte[]>> sequenceConstructionAnd(int queryID, int k,
		...
		if ((k == presentStack) && (k != 0)) {

			earray3 = connectAnd(stacks[k].eventQueue.peekLast(),
					sequenceConstructionAnd(queryID, k - 1, presentStack,
							stacks));
			for (int t = 0; t < earray3.size(); t++) {
				earray.add(earray3.get(t));
			}
	...
	public void produceand(int queryID, EventActiveInstanceQueue[] stacks,
		...
		ArrayList<ArrayList<byte[]>> sc = sequenceConstructionAnd(queryID,
				stacks.length - 1, stackIndex, stacks);
	...
	protected void generateResults(int qi, SchemaElement[] inputschArray) {
		...
		if (this.resultBuffers.get(qi) != null) {
			...
			for (ArrayList<byte[]> iresult : this.resultBuffers.get(qi)) {
			...
			if (this.resultBuffers.get(qi) != null)
				this.resultBuffers.get(qi).clear();
		}
