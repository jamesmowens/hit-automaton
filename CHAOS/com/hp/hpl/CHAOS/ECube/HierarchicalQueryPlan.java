...
public class HierarchicalQueryPlan {
	...
	//get all the event types in a bunch of queries
	...
	public ArrayList<MergedQueryInfo> getQueries() {
		return queries;
	}

	public void setqueries(ArrayList<MergedQueryInfo> q) {
		queries = q;
	}
		...
		for (int i = 0; i < stackTypes.size(); i++) {
			...
			if (i != stackTypes.size() - 1) {
				...
				for (int iter = 0; iter < allETypes.size(); iter++) {
					...
					if (Utility.semanticMatch(value, existType, tree))
